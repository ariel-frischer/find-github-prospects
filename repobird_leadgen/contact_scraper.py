...
_EMAIL_RE = re.compile(
    r"""(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|"(?:[--!#-[]-]|\[-	-])*")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\[(?:(?:(2(5[0-5]|[0-4][0-9])|1[0-9][0-9]|[1-9]?[0-9]))\.){3}(?:(2(5[0-5]|[0-4][0-9])|1[0-9][0-9]|[1-9]?[0-9])|[a-z0-9-]*[a-z0-9]:(?:[--!-ZS-]|\[-	-])+)\])""",
    re.IGNORECASE,
)
...
_BLOG_URL_RE = re.compile(
    r'(?:blog|website|homepage)[:\s]*?(https?://[^\s\'"]+)', re.IGNORECASE
)  # Simple blog/website finder
...
class ContactScraper:
    ...
    def _extract_from_text(self, text: str, existing_emails: Set[str]) -> ContactInfo:
        ...
        filtered_emails = {
            ...
            if not email.endswith(
                ("@users.noreply.github.com", "@example.com", ".png", ".jpg", ".gif")
            )
    ...
    def _get_file_content(self, repo: Repository, filepath: str) -> Optional[str]:
        try:
            content_file: ContentFile | list[ContentFile] = repo.get_contents(
                filepath
            )  # Type hint fix
            if (
                isinstance(content_file, ContentFile) and content_file.content
                ...
                print(
                    f"[yellow]Warning: Expected file, got directory listing for {filepath}"
                )
                return None
            else:
            ...
            if e.status == 404:
                ...
                print(
                    f"[yellow]Warning: GitHub error getting {filepath}: {e.status} {e.data}"
                )
            return None
    ...
    def extract(self, repo: Repository) -> ContactInfo:
        ...
        try:
            ...
            if owner.email:
                ...
                if (
                    _EMAIL_RE.match(owner.email)
                    and "@users.noreply.github.com" not in owner.email
                ):
            ...
            print(
                f"[yellow]Warning: Could not access owner details for {repo.full_name}: {e}"
            )
        ...
        final_contact = ContactInfo(
            emails=list(sorted(found_emails)), twitter=found_twitter, blog=found_blog
        )
