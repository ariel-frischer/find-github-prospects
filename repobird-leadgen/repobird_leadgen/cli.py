...
def _load_repos_from_cache(cache_file: Path, gh_instance: GitHubSearcher) -> List[Repository]:
    ...
    try:
        ...
        if not isinstance(raw_data, list):
            print(f"[red]Invalid cache file format: Expected a JSON list in {cache_file}")
            raise typer.Exit(code=1)
        ...
        for r_data in raw_data:
            try:
                ...
                print(f"[yellow]Warning: Could not rehydrate repo {r_data.get('full_name', 'Unknown')}: {e.status} {e.data}. Skipping.")
...
def _process_repo_for_summary(repo: Repository, scraper: ContactScraper) -> Optional[RepoSummary]:
    ...
    try:
        ...
        try:
            ...
            good_first_issues = repo.get_issues(state='open', labels=['good first issue']).totalCount
        except GithubException as e:
            print(f"[yellow]Warning: Could not get 'good first issue' count for {full_name}: {e.status}. Setting to 0.")
            good_first_issues = 0
        except Exception as e:
             print(f"[yellow]Warning: Unexpected error getting 'good first issue' count for {full_name}: {e}. Setting to 0.")
             good_first_issues = 0


        try:
            help_wanted_issues = repo.get_issues(state='open', labels=['help wanted']).totalCount
        except GithubException as e:
            print(f"[yellow]Warning: Could not get 'help wanted' count for {full_name}: {e.status}. Setting to 0.")
            help_wanted_issues = 0
        except Exception as e:
             print(f"[yellow]Warning: Unexpected error getting 'help wanted' count for {full_name}: {e}. Setting to 0.")
             help_wanted_issues = 0
        ...
        print(f"[yellow]Rate limit hit processing repo {repo.full_name}. Skipping for now.")
        # Re-raise or handle differently if needed
        return None # Skip this repo in parallel map
    except GithubException as e:
        print(f"[yellow]Warning: GitHub error processing repo {repo.full_name}: {e.status} {e.data}. Skipping.")
        return None
    except Exception as e:
...
@app.command()
def search(
    label: str = typer.Option("good first issue", "--label", "-l", help="Issue label to search for (e.g., 'good first issue', 'help wanted')."),
    language: str = typer.Option("python", "--language", "-lang", help="Primary programming language of the repository."),
    max_results: int = typer.Option(30, "--max-results", "-n", help="Maximum number of repositories to fetch."),
    min_stars: int = typer.Option(20, "--min-stars", "-s", help="Minimum number of stars required."),
    recent_days: int = typer.Option(365, "--recent-days", "-d", help="Repository must have been pushed to within this many days."),
    ...
    """
    print(f"[bold blue]Starting GitHub repository search...[/]")
    print(f"  Label: '{label}'")
    ...
    try:
        ...
        repos = gh.search(
         ...
         print("[red]GitHub rate limit exceeded during search. Please wait and try again, or use a token with higher limits.")
         raise typer.Exit(code=1)
    except GithubException as e:
...
@app.command()
def enrich(
    cache_file: Path = typer.Argument(..., help="JSON cache file produced by the `search` command.", exists=True, file_okay=True, dir_okay=False, readable=True),
    output_formats: List[str] = typer.Option(["md", "jsonl"], "--format", "-f", help="Output format(s): md, jsonl, csv."),
    output_dir: Path = typer.Option(OUTPUT_DIR, "--output-dir", "-o", help="Directory to save the final summary files."),
    concurrency: int = typer.Option(CONCURRENCY, "--concurrency", "-w", help="Number of parallel workers for fetching details."),
) -> None:
    """
    ...
    """
    print(f"[bold blue]Starting enrichment process...[/]")
...
def full_pipeline(
    ...
    label: str = typer.Option("good first issue", "--label", "-l", help="Issue label to search for."),
    language: str = typer.Option("python", "--language", "-lang", help="Primary programming language."),
    max_results: int = typer.Option(30, "--max-results", "-n", help="Max repositories to fetch."),
    min_stars: int = typer.Option(20, "--min-stars", "-s", help="Min stars required."),
    recent_days: int = typer.Option(365, "--recent-days", "-d", help="Pushed within days."),
    output_formats: List[str] = typer.Option(["md", "jsonl"], "--format", "-f", help="Output format(s): md, jsonl, csv."),
    output_dir: Path = typer.Option(OUTPUT_DIR, "--output-dir", "-o", help="Directory for summary files."),
    concurrency: int = typer.Option(CONCURRENCY, "--concurrency", "-w", help="Parallel workers for enrichment."),
) -> None:
    """
    ...
    with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix=".json", prefix="repobird_cache_") as tmp_file:
    ...
    try:
        ...
        if not cache_path.exists() or cache_path.stat().st_size == 0:
             ...
             if not is_empty_list:
                 print("[red]Search step failed to produce a valid cache file. Aborting.")
                 raise typer.Exit(code=1)
             else:
