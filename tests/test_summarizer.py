...
from repobird_leadgen.summarizer import to_markdown, to_jsonl, to_csv, save_summary, _format_output_path
...
def extract_markdown_table_content(md_string: str) -> List[List[str]]:
    ...
    table_lines = [line for line in lines if line.startswith('|')][1:] # Skip header line itself
...
def test_to_markdown_file_content(sample_summaries, tmp_path):
    ...
    assert "| Repo | Stars | Lang | Open Issues | Good First | Help Wanted | Last Push | Emails | Twitter | Blog |" in content
    assert "|---|---|---|---|---|---|---|---|---|---|" in content
...
def test_save_summary_markdown(mock_format_path, mock_to_markdown, sample_summaries, tmp_path):
...
@patch('repobird_leadgen.summarizer._format_output_path')
def test_save_summary_jsonl(mock_format_path, mock_to_jsonl, sample_summaries, tmp_path):
...
@patch('repobird_leadgen.summarizer.to_csv')
@patch('repobird_leadgen.summarizer._format_output_path')
def test_save_summary_multiple_formats(mock_format_path, mock_to_csv, mock_to_jsonl, mock_to_markdown, sample_summaries, tmp_path):
    ...
    def side_effect(outdir, base, ext):
        if ext == 'md': return mock_md_path
        if ext == 'jsonl': return mock_jsonl_path
        if ext == 'csv': return mock_csv_path
...
def test_save_summary_no_summaries(tmp_path):
    ...
    outdir_base = str(tmp_path / "output")
    with patch('repobird_leadgen.summarizer.to_markdown') as mock_md, \
         ...
         patch('repobird_leadgen.summarizer.console.print') as mock_print:
    ...
    mock_print.assert_called_with("[yellow]No summaries generated, skipping output file creation.")
...
def test_save_summary_invalid_format_ignored(sample_summaries, tmp_path):
    ...
    with patch('repobird_leadgen.summarizer.to_markdown') as mock_md, \
        ...
        save_summary(sample_summaries, outdir_base, ["md", "invalid", ""]) # Include invalid/empty
